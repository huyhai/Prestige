...
import com.g8.adapter.BookedAdapter;
...
public class BookedFragment extends Fragment implements OnClickListener {
	private static LinearLayout rl;
	// private DirectionalViewPager pagerAdBanner;
	private Button btnLeft;
	private Button btnRight;
	public static BookedAdapter adBannerAdapter;
	...
	public static ArrayList<AllGROModel> listImagesModelNew = new ArrayList<AllGROModel>();
	...
	private void initData() {
		...
		adBannerAdapter = new BookedAdapter(getActivity(), listImagesModelNew);
		...
		pageChangeListener = new OnPageChangeListener() {
			...
			public void onPageSelected(int position) {
				pos = position;
				if (!(UserFunctions.getInstance().getlistGRPBooked().size() == 0)) {
					tvHeight.setText(UserFunctions.getInstance().getlistGRPBooked()
							.get(pos).getHeight()
							+ " m");
					tvLanguage.setText(UserFunctions.getInstance().getlistGRPBooked()
							.get(pos).getLanguage());
					tvNameValue.setText(UserFunctions.getInstance()
							.getlistGRPBooked().get(pos).getFullName());
					tvNationality.setText(UserFunctions.getInstance()
							.getlistGRPBooked().get(pos).getNationality());
					tvWeight.setText(UserFunctions.getInstance().getlistGRPBooked()
							.get(pos).getWeight()
							+ " kg");
				}
	...
	private void getGRO() {
		...
		params.add(new BasicNameValuePair("tag", "getGroBooked"));
		UserFunctions.getInstance().getBookedGRO(getActivity(), params, true);
		// Log.v("HAI","call");
	}
	...
	BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction().equalsIgnoreCase(UserFunctions.GETGROBOOKED)) {
				if (intent.getExtras().getBoolean(ConstantValue.IS_SUCCESS)) {
					listImagesModelNew.clear();
					listImagesModelNew.addAll(UserFunctions.getInstance().getlistGRPBooked());
	...
	public void onResume() {
		...
		rlLish.setVisibility(View.GONE);
		 ...
		 intentGetKeySend.addAction(UserFunctions.GETGROBOOKED);
